...
const PostCard: React.FC<PostCardProps> = ({ post, onPostUpdate }) => {
  ...
  return (
    <article className="bg-white dark:bg-white rounded-xl p-4 sm:p-6 shadow-lg border border-gray-200 dark:border-gray-300 hover:shadow-xl transition-shadow duration-200">
      {/* Post Header */}
        ...
        <div className="flex items-start space-x-3">
          <Link to={`/user/${post.author.username}`} className="flex-shrink-0">
            <img
              ...
              className="h-10 w-10 sm:h-12 sm:w-12 rounded-full border-2 border-gray-200 dark:border-gray-300 hover:border-blue-500 dark:hover:border-blue-400 transition-colors"
            />
          </Link>
          <div className="min-w-0 flex-1">
            <div className="flex items-center space-x-2">
              <Link 
                ...
                className="font-semibold text-gray-900 dark:text-gray-900 hover:text-blue-600 dark:hover:text-blue-400 transition-colors truncate"
              >
                {post.author.name}
              </Link>
        ...
        {isAuthor && (
          <div className="relative">
            <button
              ...
              className="p-2 text-gray-600 hover:text-gray-600 dark:hover:text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-100 rounded-full transition-colors"
            >
              <MoreHorizontal className="h-5 w-5" />
            </button>
            {showMenu && (
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-100 rounded-lg shadow-lg border border-gray-200 dark:border-gray-300 z-10">
                <button
      ...
      {/* Post Actions */}
      <div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-300">
        <div className="flex items-center space-x-6">
          <button
            ...
            className={`flex items-center space-x-2 px-3 py-2 rounded-full transition-all duration-200 ${
              isLiked 
                ...
                : 'text-gray-500 dark:text-gray-600 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20'
            }`}
          >
          ...
          <button
            ...
            className="flex items-center space-x-2 px-3 py-2 rounded-full text-gray-500 dark:text-gray-600 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all duration-200"
          >
      ...
      {/* Comments Section */}
      {showComments && (
        <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-300 space-y-4">
          {/* Add Comment Form */}
          <form onSubmit={handleComment} className="flex items-start space-x-3">
            <img
              src={user?.avatar}
              alt={user?.name}
              className="h-8 w-8 rounded-full border border-gray-200 dark:border-gray-300 flex-shrink-0"
            />
            <div className="flex-1">
              ...
              <div className="flex justify-end mt-2">
                <button
                  ...
                  className="px-4 py-2 bg-blue-600 text-gray-900 rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
              ...
              <div key={comment._id} className="flex items-start space-x-3">
                <Link to={`/user/${comment.author.username}`} className="flex-shrink-0">
                  <img
                    ...
                    className="h-8 w-8 rounded-full border border-gray-200 dark:border-gray-300 hover:border-blue-500 dark:hover:border-blue-400 transition-colors"
                  />
                </Link>
                <div className="flex-1 min-w-0">
                  <div className="bg-gray-50 dark:bg-gray-100 rounded-lg p-3">
                    <Link 
                      to={`/user/${comment.author.username}`}
                      className="font-medium text-gray-900 dark:text-gray-900 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                    >
                      {comment.author.name}
                    </Link>
                    <p className="text-gray-700 dark:text-gray-700 mt-1 leading-relaxed">
                      {comment.content}
                    </p>
                  </div>
                  <div className="flex items-center justify-between mt-2">
                    <time className="text-xs text-gray-500 dark:text-gray-600">
